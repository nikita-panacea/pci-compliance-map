...
def check_chunk_relevance(chunk_text: str, image_query: str, image_path: str) -> bool:
    """
...
def map_chunk_to_control(chunk_text: str, image_query: str, image_path: str) -> dict:
    ...
    try:
        ...
        print(f"JSON decoding error: {e}. Output received: {output}")
        json_match = re.search(r'(\{.*\})', output, re.DOTALL)
        if json_match:
            try:
                mapping = json.loads(json_match.group(1))
            except json.JSONDecodeError as e:
                print(f"JSON decoding error after regex extraction: {e}.")
                return {}
        else:
            return {}
...
def process_screenshot(image_file, pdf_file):
    ...
    for title, chunk_list in processed_sections.items():
        for chunk in chunk_list:
            processed_chunks += 1
            if check_chunk_relevance(chunk, image_query, image_file):
