...
def clean_text(text):
    """
    Replace special/unusual characters with standard equivalents and normalize whitespace.
    Also removes backslashes and other unwanted symbols.
    """
    replacements = {
        "–": "-",
        "—": "-",
...
def split_text_by_heading(full_text, heading):
    """
    Splits the full text using the specified heading.
    Skips the very first occurrence of the heading, then uses each subsequent occurrence 
    to mark the start of a new section.
    
    Returns a dictionary mapping unique section keys to their corresponding text.
    """
    sections = {}
    ...
    if len(matches) < 2:
        print("Not enough occurrences of the heading found in the document.")
        return sections
    ...
    for i in range(2, len(matches)):
        ...
        end_index = matches[i+1].start() if i+1 < len(matches) else len(full_text)
        section_text = full_text[start_index:end_index].strip()
        # Label section keys starting with occurrence 1, 2, ...
        key = f"{heading} {i-1}"
...
def main(pdf_path, output_json):
    ...
    # Use the heading exactly as given to split the document.
    heading = "Requirement Description"
    print(f"Splitting document by heading: {heading}")
    sections = split_text_by_heading(full_text, heading)
    
    if not sections:
        print("No sections were extracted. Please check if the heading appears at least twice in the document.")
        return
